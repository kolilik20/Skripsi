...
<div align="center">
        ...
        <button class="btn"  style="background-color: #66BB6A">+ Tambah Admin Baru</button>
		</a>
	</div>
  ...
  <thead>
   ...
   <tbody>
    <?php 
			$query = $con->query("SELECT * FROM user"); // tampil menggunakan method query
			$nomor = 1;
          ...
          <td><?php echo $data['password']; ?></td>
					...
					<td >
						<a href="form_edit_admin.php?id=<?php echo $data['id']; ?>"><button class="btn" style="background-color: #039BE5">Edit</button></a> 
						<a href="proses_hapus_admin.php?id=<?php echo $data['id']; ?>" onclick="return confirm('Yakin hapus data?')"><button class="btn" style="background-color: #00B8D4">Hapus</button></a>					
					</td>
				</tr> <?php
			} ?>
    </tbody>
